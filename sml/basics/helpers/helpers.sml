(*  Sequoia  Copyright (C) 2020  Zan Naeem, Giselle Reis, Mohammed Hashim
    This program comes with ABSOLUTELY NO WARRANTY; for details see LICENSE.
    This is free software, and you are welcome to redistribute it
    under certain conditions; see LICENSE for details.
*)

structure helpersImpl : HELPERS =
struct
    ...
    let
      fun set_chosen ([], cmpf) = []
        | set_chosen ((x,y)::ls, cmpf) = 
          let
            val filtered = List.filter(fn (ck, _) => not (mset_eq (ck, x, cmpf)))ls
          in
            (x,y) :: set_chosen (filtered, cmpf)
          end
      val formSize = List.length(forms)
      fun choose (forms, 0, rest) = [(nil, rest@forms)]
        | choose (nil, k, rest) = []
        | choose (f::forms', k, rest) =
          case List.length(f::forms') < k of
              true => []
            | false => 
...
fun chooseDP (forms, k) =
    let val DPA = Array2.tabulate (Array2.RowMajor) (k+1, List.length(forms)+1, 
        ...
        fun choose (_, 0) = [[]]
          | choose ([],_) = []
          | choose (f::forms', k) =
            case Array2.sub(DPA, k, List.length(f::forms')) of
                SOME(a) => a
              | NONE =>
                (let 
...
fun distribute (ctx, f) = List.tabulate (List.length(ctx),
                                         fn i => 
