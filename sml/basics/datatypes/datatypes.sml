...
structure datatypesImpl : DATATYPES =
struct
...
and subforms_toString (c, nil) = ""
  | subforms_toString (c, [x]) = (
    case x of 
        Form (Con (cn), fl) => c ^ "(" ^ form_toString(x) ^ ")"
      | _ => c ^ " " ^ form_toString(x))
  | subforms_toString (c, [x,y]) = (
    case x of
        Form (Con (cn), fl) => (
       case y of
...
fun form_larger (Form(c1, l1), Atom(b)) = true
  ...
  | form_larger (Form(c1, l1), Form(c2, l2)) =
    conn_eq(c1,c2) andalso 
    let
      val (boolean1, boolean2) = 
...
(** Context Variables **)
(* That is, \Gamma, \Delta, etc.,
 * but also !\Gamma, \Box\Delta...
 *)
...
fun const_stringify (x,y,z) = 
    "(" ^ ctx_var_stringify x ^
    ",[" ^ (String.concatWith (",") (List.map(ctx_var_stringify)y)) ^
    "],[" ^ (String.concatWith (",") (List.map(ctx_var_stringify)z)) ^ "])"


(** Context **)
(* A set of context variables with a set of formulas *)
...
(** Context Structure **)
(* For more complicated contexts, such as \Gamma ; \Delta
 * or \Gamma \Uparrow \Delta, etc
 *)
...
(* Rule name, side, conclusion, and premises *)
