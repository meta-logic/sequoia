...
function table1(symbols, DBSymbols) {
    ...
    for (var i = 0; i < uniqueSymbols.length; i++) {
        table_body.innerHTML += "<tr><td id=\"t" + i.toString() + "\" >$$"+  uniqueSymbols[i] + "$$</td><td style=\"overflow: visible;\"><select class=\"ui search dropdown\" style=\"z-index: 1; position: fixed\" id=\"select-" + i +"\" ><option value=\"\">Type</option><option value=\"0\">atom</option><option value=\"1\">connective</option><option value=\"2\">unary</option><option value=\"3\">primary separator</option><option value=\"4\">separator</option></select></td></tr>"
        if (Object.keys(DBSymbols).includes(uniqueSymbols[i]) && DBSymbols[uniqueSymbols[i]] != "formula" 
                        && DBSymbols[uniqueSymbols[i]] != "set") {
            $("#select-" + i).dropdown("set text", DBSymbols[uniqueSymbols[i]])
        }
    }

    table.innerHTML += "<a onClick=\"apply()\" class=\"ui teal button\">Apply</a>"
